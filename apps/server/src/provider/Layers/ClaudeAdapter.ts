...
export interface ClaudeAdapterLiveOptions {
  ...
  readonly loadSdk?: () => Promise<Pick<typeof import("@anthropic-ai/claude-agent-sdk"), "query">>;
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const loadSdk = options?.loadSdk ?? importClaudeAgentSdk;
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const queryRuntime = yield* Effect.tryPromise({
        try: async (signal) => {
          const query = createQuery ?? (await loadSdk()).query;
